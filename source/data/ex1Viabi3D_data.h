 ...
 *   \see gridParams
 */
double STATE_MAX[dim]={2.0,2.0, 2.5};
 ...
 * \see gridParams
 */
unsigned long long int nbPointsState[dim]    =  {21,21,21};
 ...
 */
double LC= max(1.0, 2.0*alpha*STATE_MAX[1]);
/*!
...
{
  res[0]=  abs( x[0]+alpha*x[1]-x[2]);
  res[1]=0.0;
  res[2]=  max(abs(CONTROL_MAX[0]),abs( CONTROL_MIN[0]));
}
/*!
 * \var M : global bound  for the dynamics if defined
 */
double M= max( alpha*STATE_MAX[1]+STATE_MAX[2]+STATE_MAX[0],max(abs(CONTROL_MAX[0]),abs( CONTROL_MIN[0])));
/*!
