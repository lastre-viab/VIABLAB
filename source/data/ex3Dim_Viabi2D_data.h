 ...
 * \see controlParams
 */
unsigned long long int *nbPointsControl;
/*! \var STATE_MIN[i]
...
string prefix="ex3bis_viab2D-d3-";
/*
...
unsigned long long int NBC=51;
unsigned long long int NBS=31;

void loadModelData();
void loadModelData(){

  CONTROL_MIN=new double [dimc];
  CONTROL_MAX=new double [dimc];
  nbPointsControl=new unsigned long long int[dimc];
...
inline double constraintsXU( double * x, double * u ){
  ...
  double res=(sqrt(normU)-1.0<=0.0)?1.0:PLUS_INF;
  return res;
}
/*!
...
inline double constraintsX( double * x ){
  ...
  double res=( sqrt(normX)-r<=0)?1.0:PLUS_INF;
  return res;
}

#include "ex3Dim_Viabi2D_unused.h"
