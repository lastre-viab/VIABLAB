...
double xiB=2.0;
double xiS=2.0;
double gammaB=1000.0;
double gammaS=1000.0;
 ...
 *   \see controlParams
 */
double CONTROL_MIN[dimc]={-xiB,  -gammaB};
/*! \var CONTROL_MAX[dimc]
 *  \brief maximum values  for  control vector components
 *   \see controlParams
 */
double CONTROL_MAX[dimc]={xiS, gammaS};

/*! \var STATE_MIN[dim]
 *  \brief min values  for  state vector components
 *   \see gridParams
 */
double STATE_MIN[dim]={xB, 0.0, -alphaB*xiMax*xiMax};

/*! \var STATE_MAX[dim]
 *  \brief max values  for  state vector components
 *   \see gridParams
 */
double STATE_MAX[dim]={xS, muS*xMax,0.0 };


int sortieOK[dim]={0,0,0};
 ...
 * \see controlParams
 */
unsigned long long int nbPointsControl[dimc] =  {41,  41};
/*!
 * \var nbPointsState[dim]
 * number of  discretization points for the state variable
 * \see gridParams
 */
unsigned long long int nbPointsState[dim]    =  {101,101,101};
unsigned long long int dirTramage =2;
int refine=0;

int saveCoupeBound=0;
int saveCoupe=0;
int sliceDirs[dim]={0, 1, 1 };
double sliceVals[dim]={0., 1.0, 0.0};
/*!
 * \var periodic[dim]
...
unsigned long long int trajProjection[dim]={0,0,0};
double initPoint[dim]={0.5,0,0};

int scaling[dim]={0,0,0};
...
string prefix="maupertuis3D-g1000-";
...
inline void localDynBounds(double * x, double * res)
{
  ...
  res[1]= max(abs(STATE_MAX[2]),abs( STATE_MIN[2]));
  res[2]=  max(abs(CONTROL_MAX[1]),abs( CONTROL_MIN[1]));
}
/*!
...
{
  double res=-PLUS_INF;
  res= max(res, max(muB*abs(x[0])-x[1], x[1]-muS*abs(x[0])));
