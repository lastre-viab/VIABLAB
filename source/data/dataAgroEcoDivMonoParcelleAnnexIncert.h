...
inline double l_tych_fd(unsigned long long int  * x, unsigned long long int * u, unsigned long long int * v )
{
	...
	if(x[1]<TT)
	{
		//cout<< " lfunction ty " << u[0] << " " << u[1] << " tych "<< " "<< v[0]<<endl;
		res= modelData->getCyclePerturbedBalance(x[0], x[1], u[0], tyValues[v[0]])*surfaces[0]+modelData->getAnnexCycleBalance(x[1], modelData->getTimeTransition(x[1], u[0]));
	}
	else
	{
		res=0.0;
...
void loadModelData(ParametersManager *PM)
{
	...
	tyMin = cp->LIMINF_TY[0];
	tyMax = cp->LIMSUP_TY[0];
	int nbPointsTy = cp->NBPOINTS_TY[0];
	tyValues = new double[nbPointsTy];
	double tyStep = (tyMax - tyMin)/(nbPointsTy - 1);
	for(int i = 0; i<nbPointsTy; i++)
	{
		tyValues[i] = tyMin + i * (tyStep);
	}
