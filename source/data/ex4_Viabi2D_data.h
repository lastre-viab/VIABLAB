 ...
 * \see controlParams
 */
unsigned long long int nbPointsControl[dimc] =  {641};
/*!
 * \var nbPointsState[dim]
 * number of  discretization points for the state variable
 * \see gridParams
 */
unsigned long long int nbPointsState[dim]    =  {81,161};
/*
...
*/
const int nbTrajs=2;
double initPoints[dim*nbTrajs]={1.5, -0.5, 0.6, 0.5};
double initControls[dimc*nbTrajs]={0.0, 0.0};
// const int nbTrajs=1;
 ...
 */
double M=6.0;// max( alpha*STATE_MAX[0]+STATE_MAX[1],max(abs(CONTROL_MAX[0]),abs( CONTROL_MIN[0])));
/*!
...
{
   double res1= max(0.2-x[0], x[0]-2.0);
