...
ControlPicker::ControlPicker(ViabiTrajectoryHelper *vth, TrajectoryParametersManager *tpm, indexSorter_t *sorterPtr) :
	trajectoryHelper(vth), strategies
	    {
	    }
    {
...
OptionalCu ControlPicker::pickControl(Trajectory &traj, TrajectoryPoints &trajDiscrete, double rho, StrategyIndexBitFlag &flag)
    {
    return pickControlFromSubPickerList(0, traj, trajDiscrete, rho, flag);
    }

OptionalCu ControlPicker::pickControlFromSubPickerList(int subListStartIndex, Trajectory &traj, TrajectoryPoints &trajDiscrete, double rho,
    ...
    *sortIndexes = &noSort;
	...
	{
	ControlPickCriteria criteria(this, &trajSimu, &trajDiscreteSimu, i, flag, rho);
...
ControlPicker* ControlPicker::addUserPicker(TrajectoryParametersManager *params, const std::string &strategyName)
	...
	{
	spdlog::error("Could not load {} control picker, no {} function defined | {}", strategyName, factoryFunctionName, dlerror());
	return this;
	}
    UserPickStrategy *userStrategy = createStrategy(strategyIndex, params);
    if (userStrategy == nullptr)
	{
	spdlog::error("Could not create {} control picker, function {} returned nullptr", strategyName, factoryFunctionName);
	return this;
	}
...
ViabiTrajectoryHelper * ControlPicker::GetTrajectoryHelper()
    {
