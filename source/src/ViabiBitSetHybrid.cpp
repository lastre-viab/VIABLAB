...
void ViabiBitSetHybrid::InitViabiBitSetHybrid(const algoViabiParams &avbp)
{
  ...
  /*!
   * On initialise la structure  qui servira au stockage temporaire de l'image
   * discrete d'un point de grille Les tableaux sont initialis�s  avec leur
   * taille maximale possible : �gale au nb  de controles. C'est la valeur
   * effective  de nbImageCells , calcul� � chaque evaluation de l'image
   * discrete d'un point qui  servira  � lire et remplir  correctement la bonne
   * partie de ces tableaux
   */
...
void ViabiBitSetHybrid::printViabiInfo() { grid->printGrid(); }
...
void ViabiBitSetHybrid::noyauViabi(int nbArret)
{
  ...
  unsigned long long int *currentDiscreteState =
      new unsigned long long int[dim_d];
  double *currentContinuousState = new double[dim_c];

  // testK0();
bool withVectorShifts = grid->UseSubGridVectorShifts;
  while (comptEnleves > (unsigned long long int)nbArret)
  {
    ...
    for (unsigned long long int posX = 0; posX < subGridSize; posX++)
    {
      ...
      if (!gridTab[posX]->none())
      {
        ...
        masque = grid->analyseTrameMasqueWithVectorShifts(posX);
        ...
        else
        {
          ...
          for (unsigned long long int k = 0; k < longTrame; k++)
          {
            if (masque[k])
            {
              xCoordsDouble[dirTramage] =
                  limInf[dirTramage] + k * gridStep[dirTramage];
              ...
              currentContinuousState, currentDiscreteState, false);
...
spdlog::debug("Iteration {} finie nb points enleves {}", nbIter, comptEnleves);
    cout << "Itération " << nbIter
        << " terminée. Nombre de points  points enlevés: " << comptEnleves
        << "\n";
...
bool ViabiBitSetHybrid::findViabImagePoint(double *xCoordsDouble,
                                           unsigned long long int *xCoordsInt,
                                           bool print)
{
  print = false;
  ...
  /*
   * On recherche la plus grande puissance de 2 plus grande  que le nombre
   * totale de contrôles à parcourir
   */
  ...
  while ((cu_c < nbCTotal) && !testNonVide)
  {
    ...
    while ((cu_d < nbCHybridTotal) && !testNonVide)
    {
      ...
      testNonVide =
          this->CheckViability(xCoordsDouble, xCoordsInt, controlCoords[cu_c],
                               hybridControlCoords[cu_d], doubleImage, intImage,
...
bool ViabiBitSetHybrid::CheckViability(
    double *xCoordsDouble, unsigned long long int *xCoordsInt, double *uc,
    unsigned long long int *ud, double *doubleImage,
    unsigned long long int *intImage, unsigned long long int *gridCoords,
    double *tempResetc, unsigned long long int *tempResetd, double rho)
{
  ...
  if (dynsys->constraintsXU_hybrid(xCoordsDouble, xCoordsInt, uc, ud) <
      PLUS_INF)
  {

    (dynsys->*(dynsys->discretDynamics_hybrid))(xCoordsDouble, xCoordsInt, uc,
                                                ud, doubleImage, intImage,
                                                tempResetc, tempResetd, rho);
    ...
    else
    {
      testNonVide = grid->unboundedDomain &&
                    grid->isPointInGridWithConstr(doubleImage) &&
                    (dynsys->constraintsX(doubleImage) < PLUS_INF);
    }
  }
  return testNonVide;
}

void ViabiBitSetHybrid::initialiseConstraints() { setK0(); }
...
void ViabiBitSetHybrid::setK0()
{
  ...
  // calcul de la taille e prevoir pour les coordonnees des indices de debut de
  // parcours que la methode GPU va renvoyer
  ...
  for (posX = 0; posX < tailleTrame; posX++)
  {
    ...
    for (unsigned long long int k = 0; k < longTrame; k++)
    {
      ...
      xCoordsInt[dirTramage] = k;
      //	    logVector("[ViabiHybrid] : coordsDouble: ", xCoordsDouble,
      //dim); 	    logVector("[ViabiHybrid] : coordsint: ", xCoordsInt, dim);
...
string ViabiBitSetHybrid::getSetName(SetType type)
{
  ...
  transform(setName.begin(), setName.end(), setName.begin(),
...
void ViabiBitSetHybrid::saveViableSets(const string &baseFilenameSuffix)
{
  ...
  os << "../OUTPUT/" << filePrefix << "-" << setName << baseFilenameSuffix
      << ".dat";
  fileName = os.str();
  os.str("");
...
void ViabiBitSetHybrid::computeTrajectories()
{
  ...
  case CAPT:
    spdlog::error(
        "Trajectories for capture bassin problems not implemented yet");
    break;
  case VIABG:
...
void ViabiBitSetHybrid::computeViableTrajectories()
{
  ...
  //            ".dat"; fileName = os.str(); os.str("");
  //
  //            trajHelper.computeViableTrajectoryHeavy(tp->INIT_POINT,
  //            tp->INIT_CONTROL, T, fileName);
  //        }
  //        else if (typeTraj == CAUTIOUS || typeTraj ==
  //        WEIGHTED_CONTROLS_CAUTIOUS) {
  //            os << "../OUTPUT/" << filePrefix << "-traj-" << tr + 1 <<
  //            ".dat"; fileName = os.str(); os.str("");
  //
  //            trajHelper.computeCautiousTrajectory(tp->INIT_POINT, T,
  //            fileName);
  //        }
  //        else if (typeTraj == CAUTIOUS_HEAVY || typeTraj ==
  //        WEIGHTED_CONTROLS_CAUTIOUS_HEAVY) {
  //            os << "../OUTPUT/" << filePrefix << "-traj-H-" << tr + 1 <<
  //            ".dat"; fileName = os.str(); os.str("");
  //
  //            trajHelper.computeCautiousTrajectoryHeavy(tp->INIT_POINT,
  //            tp->INIT_CONTROL, T, fileName);
  //        }
  ...
  //            ".dat"; fileName = os.str(); os.str("");
  //
  //            trajHelper.computeStrategyTrajectory(tp->INIT_POINT, T,
  //            fileName);
  //        }
  //        else {
  //            spdlog::warn("Unimplemeted trajectory type. No trajectory
  //            calculation.");
  //        }
  //	    }
}
...
void ViabiBitSetHybrid::computeTychasticViableTrajectories()
{
  ...
  //            ".dat"; fileName = os.str(); os.str("");
  //
  //            trajHelper.computeTychasticStrategyTrajectory(tp->INIT_POINT, T,
  //            fileName, tychePicker);
  //        }
  //        else {
  //            spdlog::error("Unimplemented trajectory type. No trajectory
  //            calculation");
  //        }
  //    }
}
...
SysDyn *ViabiBitSetHybrid::GetSysDynForViabProblem() { return this->dynsys; }

void ViabiBitSetHybrid::saveViableSets() { saveViableSets(""); }
