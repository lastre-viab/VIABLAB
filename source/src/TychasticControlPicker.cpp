...
OptionalCu TychasticControlPicker::pickControl(TychasticTrajectory &traj,
...
OptionalCu TychasticControlPicker::pickControlFromSubPickerList(
		int subListStartIndex, TychasticTrajectory &traj,
		TrajectoryPoints &trajDiscrete, double rho,
		StrategyIndexBitFlag &flag) {
	...
	*sortIndexes = &noSort;
	...
	for (int i = subListStartIndex; i < nbStrats; ++i) {
		TychasticControlPickCriteria criteria(this, tp, &trajSimu,
				&trajDiscreteSimu, i, flag, tycheIndex, rho);
...
TychasticControlPicker* TychasticControlPicker::addPicker(
		TychasticControlPickStrategy *strategy) {

	constexpr int maxNbStrategies = 8 * sizeof(StrategyIndexBitFlag) - 1;

	if (strategies.size() >= maxNbStrategies) {
		spdlog::error(
				"Possibilty to have strategy list longer than {} strategies not supported. Sorry.",
				maxNbStrategies);
		spdlog::error(
				"All strategies past index {} will be ignored. Try creating your own personalized strategy.",
				maxNbStrategies - 1);
	} else {
...
TychasticControlPicker* TychasticControlPicker::addUserPicker(
		TrajectoryParametersManager *params, const std::string &strategyName) {
	...
	TychasticUserPickStrategy* (*createStrategy)(int strategyIndex,
			const TrajectoryParametersManager*) =
	...
	if (createStrategy == nullptr) {
		spdlog::error(
				"Could not load {} control picker, no {} function defined | {}",
				strategyName, factoryFunctionName, dlerror());
		return this;
	}
	TychasticUserPickStrategy *userStrategy = createStrategy(strategyIndex,
			params);
	if (userStrategy == nullptr) {
		spdlog::error(
				"Could not create {} control picker, function {} returned nullptr",
...
int* TychasticControlPicker::sortPreferedControlIndexes(const double *x,
		double t, int strategyIndex) {
	unsigned long long int nbCTotal = sysDyn->getTotalNbPointsC();
	double **controlCoords = sysDyn->getControlCoords();
	(*sortIndexes)(preferedControlIndexes, x, controlCoords, nbCTotal, t,
			controlWeight, trajIndex, strategyIndex);
	return preferedControlIndexes;
}
