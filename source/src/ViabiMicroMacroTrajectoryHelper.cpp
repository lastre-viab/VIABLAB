...
double ViabiMicroMacroTrajectoryHelper::computeOptimalTrajectory_tmin(
		double *initPosition, string fileName, bool &succes) {
		...
		if (dynsys->constraintsX(initPosition) < PLUS_INF) {
			...
			if (!testNonVide) {
				...
				while ((time < T) && (c >= PLUS_INF)
					...
					optimSuccessor = this->findOptimalDiscreteSuccessor_tmin(
					...
					if (optimSuccessor > grid->nbTotalPoints) {
						...
						bestCu = this->findOptiControl_tmin(xCoordsDouble,
...
int ViabiMicroMacroTrajectoryHelper::findOptiControl_tmin(double *currentPos,
...
unsigned long long int ViabiMicroMacroTrajectoryHelper::findOptimalDiscreteSuccessor_tmin(
