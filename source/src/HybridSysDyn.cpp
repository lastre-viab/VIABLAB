...
HybridSysDyn::HybridSysDyn()
    : SimpleSysDyn(), dimS_hc(0), dimS_hd(0), hybridTransistionControls(new ControlGrid()), hybridGrid(nullptr), dynamics_hybrid_d(nullptr),
      dynamics_hybrid_c(nullptr),
      resetmap_hybrid(nullptr), constraintsXU_hybrid(nullptr), constraintsX_hybrid(nullptr),
      localDynBounds_hybrid(nullptr), jacobian_hybrid(nullptr),
      calculLHybridFunc(), calculMHybridFunc(), discretDynamicsHybrid(), ownsHybridControls(true)
{
    isHybrid = true;
}

HybridSysDyn::HybridSysDyn(const systemParams& SP, int continuousStateDim, int discreteStateDim,
                           const controlParams& cp, GridBitSetHybrid* refGrid, ControlGrid* controlGrid,
                           ControlGrid* hybridControls)
    : SimpleSysDyn(SP, continuousStateDim + discreteStateDim, cp, refGrid, controlGrid), dimS_hc(continuousStateDim),
      dimS_hd(discreteStateDim),
      hybridTransistionControls(hybridControls), hybridGrid(refGrid), dynamics_hybrid_d(nullptr), dynamics_hybrid_c(nullptr),
      resetmap_hybrid(nullptr),
      constraintsXU_hybrid(nullptr), constraintsX_hybrid(nullptr), localDynBounds_hybrid(nullptr),
      jacobian_hybrid(nullptr), calculLHybridFunc(),
...
void HybridSysDyn::initializeMethods(const systemParams& SP)
{
    ...
    {
    case ANALYTICAL:
        calculLHybridFunc = [this](const double* xc, const unsigned long long int* xd)
        {
            return returnL_local_ana_hybrid(xc, xd);
        };
        break;
    case ANALYTICAL_CALC:
        calculLHybridFunc = [this](const double* xc, const unsigned long long int* xd)
        {
            return calculL_local_ana_hybrid(xc, xd);
        };
        break;
    case NUMERICAL_CALC:
        calculLHybridFunc = [this](const double* xc, const unsigned long long int* xd)
        {
            return calculL_local_num_hybrid(xc, xd);
        };
        break;
    }
    ...
    {
    case ANALYTICAL:
        calculMHybridFunc = [this](const double* xc, const unsigned long long int* xd)
        {
            return returnMF_local_ana_hybrid(xc, xd);
        };
        break;
    case ANALYTICAL_CALC:
        calculMHybridFunc = [this](const double* xc, const unsigned long long int* xd)
        {
            return calculMF_local_ana_hybrid(xc, xd);
        };
        break;
    case NUMERICAL_CALC:
        calculMHybridFunc = [this](const double* xc, const unsigned long long int* xd)
        {
            return calculMF_local_num_hybrid(xc, xd);
        };
        break;
    }
    ...
    case NO_DISCRETIZATION_SCHEME:
        discretDynamicsHybrid = [this](const double* xc, const unsigned long long int* xd, const double* uc,
                                       const unsigned long long int* ud, double* resc,
                                       unsigned long long int* resd, double* tempReset,
                                       unsigned long long int* tempResD, double rho)
        {
            FDiscret_hybrid(xc, xd, uc, ud, resc, resd, tempReset, tempResD, rho);
        };
        break;
    case EL:
        discretDynamicsHybrid = [this](const double* xc, const unsigned long long int* xd, const double* uc,
                                       ...
                                       unsigned long long int* tempResD, double rho)
        {
            FDiscretEuler_hybrid(xc, xd, uc, ud, resc, resd, tempReset, tempResD, rho);
        };
        break;
    case RK2:
        discretDynamicsHybrid = [this](const double* xc, const unsigned long long int* xd, const double* uc,
                                       ...
                                       unsigned long long int* tempResD, double rho)
        {
            FDiscretRK2_hybrid(xc, xd, uc, ud, resc, resd, tempReset, tempResD, rho);
        };
        break;
    case RK4:
        discretDynamicsHybrid = [this](const double* xc, const unsigned long long int* xd, const double* uc,
                                       ...
                                       unsigned long long int* tempResD, double rho)
        {
            FDiscretRK4_hybrid(xc, xd, uc, ud, resc, resd, tempReset, tempResD, rho);
        };
        break;
    default:
        discretDynamicsHybrid = [this](const double* xc, const unsigned long long int* xd, const double* uc,
...
void HybridSysDyn::FDiscret_hybrid(const double* xc, const unsigned long long int* xd, const double* uc,
                                   const unsigned long long int* ud, double* resc,
                                   unsigned long long int* resd, double* tempResC, unsigned long long int* tempResD,
                                   double rho) const
{
...
void HybridSysDyn::FDiscretEuler_hybrid(const double* xc, const unsigned long long int* xd, const double* uc,
                                        const unsigned long long int* ud, double* resc,
                                        unsigned long long int* resd, double* tempResC,
                                        unsigned long long int* tempResD, double rho) const
{
    FDiscret_hybrid(xc, xd, uc, ud, resc, resd, tempResC, tempResD, rho);
    for (int i = 0; i < dimS_hc; i++)
    {
        resc[i] = tempResC[i] + rho * dynSignFactor * resc[i];
    }
    hybridGrid->periodizeHybridPoint(resc);
}

void HybridSysDyn::FDiscretRK2_hybrid(const double* xc, const unsigned long long int* xd, const double* uc,
                                      const unsigned long long int* ud, double* resc,
                                      unsigned long long int* resd, double* tempResC, unsigned long long int* tempResD,
                                      double rho) const
{
    ...
    FDiscret_hybrid(xc, xd, uc, ud, Fx, resd, tempResC, tempResD, rho);
    for (int i = 0; i < dimS_hc; i++)
    {
...
void HybridSysDyn::FDiscretRK4_hybrid(const double* xc, const unsigned long long int* xd, const double* uc,
                                      const unsigned long long int* ud, double* resc,
                                      unsigned long long int* resd, double* tempResC, unsigned long long int* tempResD,
                                      double rho) const
{
    ...
    FDiscret_hybrid(xc, xd, uc, ud, ki, resd, tempResC, tempResD, rho);
    for (int i = 0; i < dimS_hc; i++)
    {
        resc[i] = tempResC[i] + rho * dynSignFactor * ki[i] / 6.0;
        y[i] = tempResC[i] + 0.5 * rho * dynSignFactor * ki[i];
    }
    hybridGrid->periodizeHybridPoint(y);
...
double HybridSysDyn::calculL_local_num_hybrid(const double* xc, const unsigned long long int* xd) const
{
    ...
    double* infX = hybridGrid->limInf;
    double* pasX = hybridGrid->step;
    double* supX = hybridGrid->limSup;
    double L1 = 0;
