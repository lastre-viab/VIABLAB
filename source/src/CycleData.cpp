...
void CycleData::computeCyclePartialData(string prefix, double * Igrid, unsigned long long int nbIPoints, unsigned long long int T)
{
	...
	for(int k=0;k<nbIPoints;k++)
	{
		...
		for(int t=0; t<globalCycleDuration-1;t++)
		{
			...
			{
				iNextIndex = (unsigned long long int)min((int)nbIPoints - 1, max(0, (int)round((tempI-Igrid[0])/(Igrid[1]-Igrid[0]))));
				//iNextIndex = (unsigned long long int)min((int)nbIPoints - 1, max(0, (int)floor((tempI-Igrid[0])/(Igrid[1]-Igrid[0]))));
	...
	else
	{
		...
		while(currentLongM<=duree)
		{
			...
			while((shortCropIndex < this->nbShortCrops)  && (currentLongM<=duree))
			{
				...
				double ILongEstimation = initI+min(currentLongM-1, longCropDuration)*longCropIStep;//estimation du IBQS interm�diaire de la spec longue
...
double CycleData::calculDeltaBilanMoisCycle(unsigned long long int m, double initI, double v)
{
	double res = 0.0;
	if(cycleType==0)
	{
		res = longCropData->calculDeltaBilanMois(m, initI, v);
	}
	else
	{
		unsigned long long int time=0;
		...
		while(currentLongM<=duree)
		{
			...
			while((shortCropIndex < this->nbShortCrops)  && (currentLongM<=duree))
			{
				...
				while((currentShortM <= shortCropDuration) && (currentLongM<=duree))
				{
					//cout << " currentLongM = "<< currentLongM << " duree "<< duree << endl;
					//cout << " currentShortM = "<< currentShortM << " nb shortCrop  "<< shortCropsData[shortCropIndex]->name << endl;
					bilanTemp = this->calculDeltaBilanMoisMixte(initI, I, currentLongM, currentShortM, currentShortCrop, v);
					//cout<< " \n le bilan du mois " << bilanTemp << " revenu cumule "<<revenuCumul<< endl;
					currentLongM++;
					currentShortM++;
				}
...
double CycleData::calculBilanCycleSingleCrop(double I,int t, int T)
{
	...
	for(unsigned long long int  k=1;k<=duree;k++)
	{
		...
		// cout<< " mois "<<k<< " bilan cumulé "<<bl<<endl;
	}
	//cout<< "=======================\n";
	return bl;
}

double CycleData::calculDeltaBilanCycleSingleCrop(double I,int t, int T, double v)
{
	double bl=0.0;
	unsigned long long int duree;
	if(t+globalCycleDuration<=T)
	...
	for(unsigned long long int  k=1;k<=duree;k++)
	{
		...
		bl=bl+longCropData->calculDeltaBilanMois(k, I, v);
		// cout<< " mois "<<k<< " bilan cumulé "<<bl<<endl;
	}
	//cout<< "=======================\n";
	return bl;
...
double CycleData::calculBilanCycleMixedCrop(double initI,int t, int T)
{
	...
	while(currentLongM<=duree)
	{
		...
		while((shortCropIndex < this->nbShortCrops)  && (currentLongM<=duree))
		{
			...
			CropData * currentShortCrop = this->shortCropsData[shortCropIndex];
			unsigned long long int shortCropDuration = currentShortCrop->duration;
			unsigned long long int currentShortM=1;
			while((currentShortM <= shortCropDuration) && (currentLongM<=duree))
			{
				//cout << " currentLongM = "<< currentLongM << " duree "<< duree << endl;
...
double CycleData::calculDeltaBilanCycleMixedCrop(double initI,int t, int T, double v)
{
	...
	while(currentLongM<=duree)
	{
		...
		while((shortCropIndex < this->nbShortCrops)  && (currentLongM<=duree))
		{
			...
			while((currentShortM <= shortCropDuration) && (currentLongM<=duree))
			{
				//cout << " currentLongM = "<< currentLongM << " duree "<< duree << endl;
				//cout << " currentShortM = "<< currentShortM << " shortCrop  "<< shortCropsData[shortCropIndex]->name << endl;
				double bilanTemp = this->calculDeltaBilanMoisMixte(initI, I, currentLongM, currentShortM, currentShortCrop, v);
				revenuCumul+= bilanTemp;
				//cout<< " \n le bilan du mois " << bilanTemp << " revenu cumule "<<revenuCumul<< endl;
				currentLongM++;
				currentShortM++;
...
double CycleData::calculDeltaBilanMoisMixte(double Ilong, double Ishort, unsigned long long int mLong, unsigned long long int mShort, CropData * shortCrop, double v)
{
	//cout<< " fixedCost = "<<fixedCost<<endl;

	double bilanL=  longCropData->calculDeltaBilanMoisAssocie(mLong, Ilong, fixedCost, weightLong, LERLong, v);
	double bilanC= shortCrop->calculDeltaBilanMoisAssocie(mShort, Ishort, fixedCost, 1.0-weightLong, LERShort, v);
	//cout<< " bilan mois mixte mois court :  "<< bilanC << " mois long "<< bilanL<< endl;
	return bilanL+bilanC;
...
double CycleData::calculDeltaBilanCycle(double I,int t, int T, double v)
{
	return (cycleType==0)? this->calculDeltaBilanCycleSingleCrop(I, t, T, v):this->calculDeltaBilanCycleMixedCrop(I, t, T, v);
}
...
void CycleData::saveCyclePartialData(string prefix, unsigned long long int nbIPoints, double * Igrid)
{
	...
	else
	{
		...
		for(int l1=0;l1<nbIPoints;l1++)
		{
			double IbqsComplet = computeNextIBQSValue(Igrid[l1], 0, globalCycleDuration+100);
			 unsigned long long int index =(unsigned long long int)min((int)nbIPoints - 1, max(0, (int)round((IbqsComplet-Igrid[0])/(Igrid[1]-Igrid[0]))));
			//unsigned long long int index =(unsigned long long int)min((int)nbIPoints - 1, max(0, (int)floor((IbqsComplet-Igrid[0])/(Igrid[1]-Igrid[0]))));
						fprintf(fi,  "%d " ,    index);
			for(int t=0; t<globalCycleDuration-1;t++)
			{
	...
	os<<"../OUTPUT/"<<prefix<<"-"<<cycleName<<"-PartialIBQSPrevious.dat";
		...
		else
		{
			...
			{
				//double IbqsComplet = computeNextIBQSValue(Igrid[l1], 0, globalCycleDuration+100);
				//unsigned long long int index =(unsigned long long int)max(0,(int)round((IbqsComplet-Igrid[0])/(Igrid[1]-Igrid[0])));
				//fprintf(fi,  "%d " ,    index);
				for(int t=0; t<globalCycleDuration-1;t++)
				{
