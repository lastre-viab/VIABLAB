...
int ViabiBitSetTrajectoryHelper::findViabControl(double *currentPos, double &dt, int nbStepIter, double stepCoeff, double *resPos, int &nbViabVoisins,
	bool &succes)
    {
	...
	while ((cu < nbCTotal) && !testviabInt)
	     ...
	     * au point en cours
	     */
	    if (isViableControl(xCoordsDouble, controlCoords[cu], imageVect, rho))
		{
		/*
...
unsigned long long int ViabiBitSetTrajectoryHelper::findViableDiscreteSuccessor(unsigned long long int pos, double time, double dt, int &usedCu)
	 ...
	 * au point en cours
	 */
	if (isViableControl(xCoordsDouble, controlCoords[preferedControlIndexes[cPrefu]], imageVect, rho))
	    {
...
unsigned long long int ViabiBitSetTrajectoryHelper::findViableDiscreteSuccessor_tych(unsigned long long int pos, double time, double dt,
	int tychIndex, int &usedCu)
    {
    unsigned long long int viabSuccessor = grid->nbTotalPoints + 1;
    unsigned long long int intCoordsOnDiscreteTraj[dim];
    /*
	 ...
	 * au point en cours
	 */
	if (isViableControl_tych(xCoordsDouble, controlCoords[preferedControlIndexes[cPrefu]], tycheCoords[tychIndex], imageVect, rho))
	    {
	    viabSuccessor = grid->getNearestPointInSet(imageVect);
	    testNonVide = (viabSuccessor < grid->nbTotalPoints + 1);
	    usedCu = preferedControlIndexes[cPrefu];
...
int ViabiBitSetTrajectoryHelper::findViabControl_bis(const double *currentPos, unsigned long long int optimDiscreteSuccessor, double &dt,
...
int ViabiBitSetTrajectoryHelper::findViabControl_bis_tych(const double *currentPos, unsigned long long int optimDiscreteSuccessor, double &dt,
	int tychIndex, int nbStepIter, double stepCoeff, double *resPos, bool &succes)
    {
    double realCoordsOnDiscreteTraj[dim];
    unsigned long long int intCoordsOnDiscreteTraj[dim];
    double rho0 = dt;
	...
	while (cu < nbCTotal)
	     ...
	     * au point en cours
	     */
	    if (isViableControl_tych(xCoordsDouble, controlCoords[cu], tycheCoords[tychIndex], imageVect, rho))
		{
		unsigned long long int cellNum = grid->localizePoint(imageVect);
...
int ViabiBitSetTrajectoryHelper::computeViableTrajectory(double *initPosition, double finalTime, string fileName)
    ...
    ViabiTrajectoryHelper::PointStatus initStatus = checkKernelRelation(initPosition);
    if (initStatus == OUTSIDE_DOMAIN)
	{
	spdlog::error("Initial point is out of the viability domain. Stop.");
	}
    else if (initStatus == OUTSIDE_CONSTRAINTS)
	{
	spdlog::error("Initial point is out of the constraints set. Stop.");
	}
    else if (initStatus == OUTSIDE_GRID)
	{
	...
	while (succes && time < finalTime && nbIter < NB_MAX_TRAJ_ITER)
	    {
	    succes = computeViableTrajectoryIteration(currentDiscreteTrajPos, xCoordsDouble, currentPosIntCoords, doubleCoordsOnDiscreteTraj,
		    imageVect, finalTime, time, traj, trajDiscrete, realTimeStepsPerDiscreteStep);
	    nbIter++;
	    }
	if (time < finalTime)
	    {
...
bool ViabiBitSetTrajectoryHelper::findPositionAtBorder(double *xCoordsDouble, Bubble &bubble, double &futureTime, double &rho, bool &onBorder,
	unsigned long long int &discreteBorderPos, double *borderCoords, int realTimeStepsPerDiscreteStep)
    {
    ...
    while (bubble.isInBubble(currentPosIntCoords) && !onBorder && nbIter < NB_MAX_TRAJ_SIMULATIONS)
	{

	if (!computeViableTrajectoryIteration(discreteBorderPos, simulatedFuturePos, currentPosIntCoords, doubleCoordsOnDiscreteTraj, imageVect,
		PLUS_INF, rho, traj, trajDiscrete, realTimeStepsPerDiscreteStep))
	    {
	    return false;
	    }
...
bool ViabiBitSetTrajectoryHelper::computeViableTrajectoryIteration(unsigned long long int &currentDiscreteTrajPos, double *xCoordsDouble,
	unsigned long long int *currentPosIntCoords, double *doubleCoordsOnDiscreteTraj, double *imageVect, double finalTime, double &time,
	Trajectory &traj, TrajectoryPoints &trajDiscrete, int realTimeStepsPerDiscreteStep)
    {
    bool testNonVide;
    /*
     * tableau de coordonnées  définissant
...
int ViabiBitSetTrajectoryHelper::applyClosestToControl(unsigned long long int currentDiscreteTrajPos, int cu, double rho, double time,
	double *xCoordsDouble, double *imageVect)
    {
    using ull = unsigned long long int;
	...
	viableControlFound = isViableControl(xCoordsDouble, prefControl, imageVect, rho);
	prefCu--;
	}
...
int ViabiBitSetTrajectoryHelper::computeCautiousTrajectory(double *initPosition, double finalTime, string fileName)
	...
	while (succes && time < finalTime && nbIter < NB_MAX_TRAJ_ITER)
		...
		if (!findPositionAtBorder(xCoordsDouble, bubble, futureTime, futureRho, onBorder, discreteBorderPos, borderCoordsDouble,
		...
		if (!onBorder)
		    {
		    succes = computeViableTrajectoryIteration(currentDiscreteTrajPos, xCoordsDouble, currentIntCoords, doubleCoordsOnDiscreteTraj,
			    imageVect, finalTime, time, traj, trajDiscrete, realTimeStepsPerDiscreteStep);
		    }
		else
		    {
			...
			if (!foundBorderCu)
			    {
			    spdlog::warn(
				    "No viable real control found on border with approximated real trajectory position. Using control from discrete grid position instead");
			    borderCu = usedCu;
			    }
			rho = realTimeStep;
			}
		    ...
		    if (!isViableControl(xCoordsDouble, controlCoords[borderCu], imageVect, rho))
			{
			if ((ull) (borderCu = applyClosestToControl(currentDiscreteTrajPos, borderCu, rho, time, xCoordsDouble, imageVect))
				>= nbCTotal)
			    {
			    succes = false;
			    break;
...
bool ViabiBitSetTrajectoryHelper::findPositionAtBorderHeavy(double *xCoordsDouble, const double *control, Bubble &bubble, double &futureTime,
	double &rho, bool &onBorder, unsigned long long int &discreteBorderPos, double *borderCoords)
    {
    using ull = unsigned long long int;
	...
	bool viable = isViableControl(xCoordsDouble, control, imageVect, rho);
	bool differentGridPos = grid->getNearestPointInSet(imageVect) != discreteBorderPos;
	while (!viable && differentGridPos)
	    {
	    rho /= 2;
	    viable = isViableControl(xCoordsDouble, control, imageVect, rho);
...
int ViabiBitSetTrajectoryHelper::computeCautiousTrajectoryHeavy(double *initPosition, double *initControl, double finalTime, string fileName)
	...
	while (succes && time < finalTime && nbIter < NB_MAX_TRAJ_ITER)
		...
		ull discreteBorderPos;
		bool onBorder = false;
		findPositionAtBorderHeavy(xCoordsDouble, currentControl, bubble, futureTime, futureRho, onBorder, discreteBorderPos,
			borderCoordsDouble);
		...
		if (!onBorder)
		    ...
		    if (isViableControl(xCoordsDouble, currentControl, imageVect, futureRho))
			{
		    ...
		    // Si le contrôle au bord n'est pas viable, on prend le premier contrôle viable
		    // en partant de celui du bord vers celui de l'emplacement actuel
		    if (!isViableControl(xCoordsDouble, currentControl, imageVect, rho))
			{
			if ((ull) (currentCu = applyClosestToControl(currentDiscreteTrajPos, currentCu, rho, time, xCoordsDouble, imageVect))
				>= nbCTotal)
			    {
			    succes = false;
			    break;
	    ...
	    else if (isViableControl(xCoordsDouble, currentControl, imageVect, rho))
		{
		time += rho;
		std::copy(imageVect, imageVect + dim, xCoordsDouble);
		currentDiscreteTrajPos = grid->getNearestPointInSet(xCoordsDouble);
...
int ViabiBitSetTrajectoryHelper::computeStrategyTrajectory(double *initPosition, double finalTime, string fileName)
	...
	{
	ControlPickerBitSet picker(this, tpm);
	double **controlCoords = dynsys->getControlCoords();
	...
	grid->numToIntAndDoubleCoords(currentDiscreteTrajPos, currentPosIntCoords, doubleCoordsOnDiscreteTraj);
	TrajectoryStorage traj =
		(tpm->getTrajectoryParameters()->SAVE_PICKING_STRATEGY) ?
	...
	while (succes && time < finalTime && nbIter < NB_MAX_TRAJ_ITER)
		...
		// structured binding
		    ...
		    if (!isViableControl(xCoordsDouble, controlCoords[cu], imageVect, pickedRho))
			{
...
int ViabiBitSetTrajectoryHelper::computeTychasticStrategyTrajectory(double *initPosition, double finalTime, string fileName, TychePicker &tychePicker)
	...
	{
	TychasticControlPickerBitSet picker(this, tpm);
	double **controlCoords = dynsys->getControlCoords();
	double **tycheCoords = dynsys->getTychCoords();
	...
	unsigned long long int currentDiscreteTrajPos = grid->getNearestPointInSet(xCoordsDouble);
	grid->numToIntAndDoubleCoords(currentDiscreteTrajPos, currentPosIntCoords, doubleCoordsOnDiscreteTraj);
	TychasticTrajectoryStorage traj =
		(tpm->getTrajectoryParameters()->SAVE_PICKING_STRATEGY) ?
	...
	while (succes && time < finalTime && nbIter < NB_MAX_TRAJ_ITER)
		...
		// structured binding
		    ...
		    if (!isViableControl_tych(xCoordsDouble, controlCoords[cu], tycheCoords[tyIndex], imageVect, pickedRho))
			{
			spdlog::info("No viable real control found for strategy's pick.");
			return OptionalCu(UNSATISFIED_STRATEGY);
			}
...
ViabiTrajectoryHelper::PointStatus ViabiBitSetTrajectoryHelper::checkKernelRelation(double *position) const
    {
    int cptOK = 0;
    int cellNum;
    int posTemp;
    ...
    const long long int *indicesDecalCell = grid->getIndicesDecalCell();
    int pow2 = (int) pow(2.0, dim);

    /*
     * tableaux temporaires pour récupérer les indices du point dans la
     * grille
     */
    long long unsigned *testI = new long long unsigned[dim];
    ...
    else if (dynsys->constraintsX(position) >= PLUS_INF)
	{
