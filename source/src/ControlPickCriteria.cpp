...
ControlPickCriteria::ControlPickCriteria(ControlPicker *picker, SysDyn *sysDyn, Trajectory *traj, TrajectoryPoints *trajDiscrete, int strategyIndex, StrategyIndexBitFlag &flag) 
    : ControlPickCriteria(
        picker,
        sysDyn,
        traj, trajDiscrete,
...
ControlPickCriteria::ControlPickCriteria(ControlPicker *picker, SysDyn *sysDyn, Trajectory *traj, TrajectoryPoints *trajDiscrete, int strategyIndex, StrategyIndexBitFlag &flag, double timeStep) :
    picker(picker),
    sysDyn(sysDyn),
    traj(traj),
...
const int *ControlPickCriteria::sortPreferedControlIndexes() const {
    return picker->sortPreferedControlIndexes(
        getCurrentPos(),
        getTime(),
        strategyIndex);
}
...
bool ControlPickCriteria::isViableControl(pickedControl &pickedControl, double *imageVect) const {
    ...
    return sysDyn->isViableControl(getCurrentPos(), control, imageVect, rho);
}
