...
class ViabiBitSetTrajectoryHelper final : public ViabiTrajectoryHelper
    {
public:
    ...
    int findViabControl_bis_tych(const double *currentPos, unsigned long long int optimDiscreteSuccessor, double &dt, int tychIndex, int nbStepIter,
	    double stepCoeff, double *resPos, bool &succes);
    virtual PointStatus checkKernelRelation(double *position) const;

private:
    bool computeViableTrajectoryIteration(unsigned long long int &currentDiscreteTrajPos, double *xCoordsDouble,
    ...
    bool findPositionAtBorder(double *xCoordsDouble, Bubble &bubble, double &futureTime, double &futureRho, bool &onBorder,
	    unsigned long long int &discreteBorderPos, double *borderCoords, int realTimeStepsPerDiscreteStep);
    bool findPositionAtBorderHeavy(double *xCoordsDouble, const double *control, Bubble &bubble, double &futureTime, double &futureRho,
	    bool &onBorder, unsigned long long int &discreteBorderPos, double *borderCoords);
    int applyClosestToControl(unsigned long long int currentDiscreteTrajPos, int cu, double rho, double normalizedTime, double *xCoordsDouble,
	    double *imageVect);
