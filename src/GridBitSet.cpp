...
void Grid_BitSet::findNearestViabPointInCell(double * currentPoint, double * newPoint)
{
  ...
  if(testBord)
    ...
    if(laTrame.none())
      ...
      if((k<pow3Sub) & (posX+indicesDecalSub[k]<nbPointsTotalSubGrid))
        ...
        if(x>0.75 && x<0.95 )
          {       cout<<" masque  decal�"<<masqueDecale<<"\n";}
        masqueDecale[longTrame-1]=1;
        masque&=(masqueDecale);
        if(x>0.75 && x<0.95 )
          {cout<<" masque  apr�s d�calage"<<masque<<"\n";}
        //on d�cale la trame de 1 bite  et on fait le ET pour chaque
        //bit �a donne : b[i] ET b[i-1]
        masqueDecale=((*gridTab[posX+indicesDecalSub[k]]))<<(1);
        if(x>0.75 && x<0.95 )
        ...
        while(k<pow3Sub)
          ...
          if(posX+indicesDecalSub[k]>=0)
            ...
            masque&=(((*gridTab[posX+indicesDecalSub[k]])));
            if(x>0.75 && x<0.95 )
              {cout<<" voisin  k= "<<k<<" trame voisine "<<((*gridTab[k]))<<"\n";}
            //on d�cale la trame de 1 bite  et on fait le ET pour chaque
            //bit �a donne : b[i] ET b[i+1]
            masqueDecale=((*gridTab[posX+indicesDecalSub[k]]))>>(1);
            masqueDecale[longTrame-1]=1;
            ...
            masque&=(masqueDecale);
            if(x>0.75 && x<0.95 )
              {cout<<" voisin  k= "<<k<<" masque apr�s le straitement de ce voisi "<<masque<<"\n";}
