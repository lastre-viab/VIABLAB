        ...
        while(time<finalTime && nbIter<=NB_MAX_TRAJ_ITER)
          ...
          if(testNonVide)
            ...
            if(testviabInt)
              ...
              grid->findNearestViabPointInCell(doubleVect1, xCoordsDouble);
              for(int dc=0;dc<dimC;dc++)
                {
                trajControlCoords[dc]=currentControl[dc];
                }
            ...
            if(testNonVide)
              ...
              if(testviabInt)
                ...
                {
                cout<< " ======================= Recalage =======================\n";
                grid->findNearestViabPointInCell(imageVect, xCoordsDouble);
                cout<< " controle ";
                for(int dc=0;dc<dimC;dc++)
                  {
                  trajControlCoords[dc]=controlCoords[bestCu][dc];
...
void ViabiBitSet::noyauViabiGaranti_FD( bool sortieOK,int nbArret)
{
  ...
  while( comptEnleves>nbArret)
    ...
    //cout<<"dim etat  " <<dim<< " taille de trame est " <<tailleTrame<<"\n";
    int dirParcours=-1;
    for(  int posX=0+((1-dirParcours)/2)*(tailleTrame-1);dirParcours*(posX-((1+dirParcours)/2)*(tailleTrame-1))<=0;posX+=dirParcours)
      {
      // cout<< " posX="<<posX<<endl;
      if(!(*gridTab[posX]).none())
        {
