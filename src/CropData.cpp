...
CropData::CropData(string fichierSource) {
	...
	//elimination des commentaires dans les fichiers d'entrÃ©e
...
unsigned long long int CropData::calculINextIndex(double I, double stepI, double IMin, unsigned long long int nbPointsI)
{
	...
	if(Iplus<IMin)
	{
		...
		niPlus=max(0,(int)floor((Iplus-IMin)/(stepI)));
...
// calcul du bilan financier /mois et /par Ha pour la spéc donnée
// sans les coûts fixes, gérés au niveau de l'exploitation
double CropData::calculBilanMois(unsigned long long int t, double I, double fixedCost)
{
...

	{
		...
		{
			if(t%croppingPeriod==0)//mois de rï¿½colte
			{
